...
class Storage:
    ...
    def __enter__(self):
        ...
        self.conn.execute(queries.MAKE_TABLES)
        return self
    ...
    def save_file(self, filename, parent_dir, permissions, sha256):
        file_record = self.conn.execute(
            "SELECT * from file where dir_id = ? and filename = ?", (parent_dir,
                                                                     filename)).fetchone()
        if not file_record:
            # insert
            ...
            self.conn.execute(
                "UPDATE file set last_modified = ?, permissions = ?, sha256 = ? where id = ?",
    ...
    def old_dirs(self, root_id):
        # select all dirs
        dir_cache = {}
        result = self.conn.execute(
            "SELECT id, parent_dir_id, dir_path from directory where parent_dir_id = ?",
            (root_id,))

        for id, parent_dir_id, dir_path in result:
            dir_cache[dir_path] = id
        return dir_cache

    def old_files(self, root):
        result = self.conn.execute(
            "SELECT file.filename,file.id from file join directory "
            "on dir_id=directory.id where directory.dir_path = ?",
            (root,))
        file_cache = {}
